...
代码基于 smallpt，实现路径追踪（PT）、随机渐进式光子映射（SPPM）、CUDA Hybrid SPPM、分层复用 Hybrid SPPM、低差异 Guided Hybrid SPPM，以及面向原生 4K 的路径统计自适应纠偏缓存。原始花瓶场景与原 `balls` 分支的玻璃球场景都在 `master`，通过 `--scene vase` / `--scene balls` 直接切换。旋转 Bezier 曲面、彩色纹理、镜面反射、玻璃折射和光源构图均保留原始设定。课程报告见 [hw2/report.pdf](hw2/report.pdf)。
...
![Balls 场景：原生 4K 路径统计自适应纠偏缓存直接渲染](docs/render/balls-cache-4k.png)
...
`--cache` 在 Guided Hybrid 的物理传输分解之上增加八个变化：

1. **按分辨率控制光子支持域。** 令 $s=\min(1,\sqrt{640\cdot360/(WH)})$，自动使用 $r'=r\max(0.24,s)$、$\rho'=\rho s$，避免高分辨率下对同一世界坐标邻域做二次增长的重复查询。
2. **把混合 BSDF 明确拆成两个估计量。** 漫反射分支交给 SPPM，并乘以 $1-p_{\mathrm{specular}}$；镜面分支由独立相机路径估计，再乘回 $p_{\mathrm{specular}}$。大范围照度缓存尽量放在方向性光路合成之前；方向性分量只允许与实际几何和路径行为相容的近邻复用。
3. **缓存可复用的辐照度，而不是直接糊 RGB。** 先除掉纹理反照率，再用跨度为 1、2、4、8……像素的几何约束 à-trous 层级复用漫反射照明，最后恢复原像素纹理。物体 ID、法线、世界平面距离、反照率和实际辐亮度差异共同限制复用，不根据固定 RGB 值判断像素是不是高光。
4. **根据实际光路判断是否值得复用。** 每个像素先用独立 pilot 测量路径亮度、方差以及镜面/折射事件的出现频率。稳定、方向性弱的像素直接保留本像素结果；稀有方向事件多、采样方差大的像素才进行受几何约束的复用。阈值从 0.05 扫到 0.50 时 4K 结果保持一致，不依赖场景名、球的位置、贴图文件或预设颜色。
5. **pilot 只决定预算，不参与最终估计。** 最终镜面和折射颜色重新使用一批独立路径计算，避免“先看 pilot 决定停止，再把同一批 pilot 拿来平均”的 optional-stopping 偏差。带贴图遮罩的材质和普通玻璃共享用户指定的 `--hybrid-samples`，不再存在偷偷的 64-sample 上限。
6. **在原生像素分辨率纠正缓存偏差。** 当 `--hybrid-samples >= 128` 时，每个漫反射像素单独生成随机化低差异 PT 样本，在同一个原生 4K 像素位置计算线性辐亮度残差 $P_h-C_h$；再根据几何、法线、平面距离和实际辐亮度差异复用相容残差：

$$\hat L_h=C_h+\mathcal U_{\mathrm{compatible}}\!\left[P_h-C_h\right].$$

这里不再假定半分辨率残差能够代表 4K 细焦散，也不根据光源是否被包裹决定是否纠偏；只由用户显式提供的采样预算决定是否启用这项额外计算。

7. **不连续像素回退到自己的无偏光路。** 物体轮廓、法线/材质边界和贴图反照率跳变由实际相邻像素检测；这些位置不使用邻居的辐亮度，而是用原生分辨率、随机化低差异子像素和 BSDF 样本重新追踪自己的完整路径，避免彩条、玻璃边缘和镜面轮廓出现光晕。
8. **显式承认仍然存在的偏差。** 单条 PT 光路及 PDF 补偿是无偏的，但有限光子半径和有限邻域复用仍然是偏差—方差折中，最终图像不能宣称“逐像素无偏”。因此每次改动都同时报告跨 seed 噪声和相对于同场景高采样 PT 的系统性偏差。
...
| 花瓶 | **路径自适应缓存** | **24** | **4** | **41.697 s** | **0.292** | — | — |
...
| Balls | 上一代半分辨率纠偏 | 24 | 4 | 35.399 s | 0.430 | 0.511 | 0.668 |
| Balls | 上一代半分辨率纠偏 HQ | 24 | 4 | 71.154 s | 0.293 | 0.492 | 0.573 |
| Balls | **路径自适应缓存** | **24** | **4** | **74.196 s** | **0.460** | **0.265** | **0.531** |
| Balls | **路径自适应缓存 HQ** | **24** | **4** | **119.768 s** | **0.350** | **0.240** | **0.424** |
| Balls | **路径自适应缓存 Ultra** | **24** | **4** | **208.575 s** | **0.292** | **0.234** | **0.374** |
...
| 上一代半分辨率纠偏，24 轮 | 0.511 | 1.222 | 0.747 | 0.364 | 0.962 | 37.96 |
| 上一代半分辨率纠偏 HQ，24 轮 | 0.492 | 1.220 | 0.742 | 0.276 | 0.961 | 37.94 |
| **路径自适应缓存，24 轮** | **0.265** | **0.364** | **0.225** | **0.348** | **0.374** | **38.35** |
| **路径自适应缓存 HQ，24 轮** | **0.240** | **0.353** | **0.211** | **0.241** | **0.382** | **38.30** |
| **路径自适应缓存 Ultra，24 轮** | **0.234** | **0.346** | **0.209** | **0.230** | **0.385** | **38.29** |

![无偏 PT 参考、上一代半分辨率缓存与路径统计自适应缓存的同一区域直接对照](docs/render/balls-fidelity-comparison.png)

Ultra 档的 balls 背景 / 地板 / 倒影 / 玻璃 / 条纹随机噪声 RMS 分别为 **0.296 / 0.305 / 0.295 / 0.392 / 0.066**，整图随机噪声 **0.292**、系统性偏差 **0.234**；页面顶部展示这一档的单次直接渲染。相对于上一代 HQ，小球焦散偏差降低 **3.53×**，地板倒影降低 **3.55×**，彩条降低 **2.50×**，综合误差从 **0.573 降到 0.374**；代价是 4K GPU 时间从 71.154 s 增加到 208.575 s。更快的标准档仍比 16 轮 Guided Hybrid 快 **1.58×**，并把综合误差从 **1.771 降到 0.531**。花瓶保持 **41.697 s / RMS 0.292**。完整逐区域亮度、边缘、噪声与参考偏差见 [docs/render/convergence-4k.csv](docs/render/convergence-4k.csv)。
...
5. 使用同一个二进制渲染普通 SPPM、Guided Hybrid，或最新的原生 4K 路径统计自适应缓存：
...
# 原生 4K Balls：24 轮；原生分辨率 PT 自动校正焦散和间接光
...
# 原生 4K Balls HQ：增加玻璃和无偏 PT 样本
./render-cuda 3840 2160 balls-cache-hq-4k.ppm 24 8 .5 1 \
  --scene balls --nearest --hybrid-samples 576 \
  --reconstruction-radius 4 --cache

# 原生 4K Balls Ultra：README 展示这一档；随机 RMS 0.292、参考偏差 0.234
./render-cuda 3840 2160 balls-cache-ultra-4k.ppm 24 8 .5 1 \
...
前四个位置参数表示 `宽度 高度 输出文件 每子像素采样数`，对应 PT；再提供 `每像素光子数 初始半径 alpha` 就切换到 SPPM。`--reuse` 打开分层复用，`--guided` 增加直接光分解和低差异发光，`--cache` 进一步开启路径统计自适应缓存；当宽度至少 1280 且 `--hybrid-samples >= 128` 时，对任意场景启用原生分辨率控制变量和无偏边界重采样。都不加则保留原来的 Hybrid SPPM。CUDA SPPM 只输出最终结果。多卡机器用 `CUDA_VISIBLE_DEVICES=0` 指定 GPU。
...
python3 benchmark.py --binary ./render-cuda --algorithms cache,cache_hq,cache_ultra \
...
| 路径统计自适应缓存 | `--cache` | 包含 `--guided`，另加路径事件统计、原生分辨率 PT 控制变量和无偏边界重采样 |
